...
# --- SETTING OFFSET (Ubah angka ini untuk menggeser semua titik) ---
# Jika lingkaran merah masih di kanan, kurangi OFFSET_X (misal: -50)
# Jika lingkaran merah masih di bawah, kurangi OFFSET_Y (misal: -30)
OFFSET_X = -60  
OFFSET_Y = -40  
# -------------------------------------------------------------------
...
def proses_gambar(image):
    ...
    for no, pilihan in KOORDINAT.items():
        ...
        for opsi, (x_raw, y_raw) in pilihan.items():
            # Menerapkan offset agar titik bergeser otomatis
            x = x_raw + OFFSET_X
            y = y_raw + OFFSET_Y
...
if uploaded_file:
    if st.button("Mulai Koreksi"):
        ...
        st.json(hasil)
        st.image(cv2.cvtColor(img_visual, cv2.COLOR_BGR2RGB))
